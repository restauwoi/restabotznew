	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `©Created : Resta Botz`, 
                            orderTitle: `©Created : Resta Botz`,
...
switch (command) {
	case 'menu':
	gambar = fs.readFileSync('./zeroyt7/menu.jpg')
                   timestamp = speed();
...
└⬣`
teks =
`Ini Sc buatan © Resta Botz dan base dari Zeroyt7`
but = [
...
*┗━━━━━━━ •*`
        ...
        sendButton(from, teks, '©Created : Resta Botz', but, mek)
break
...
reply('loading tod')
buffer = getBuffer(`https://apikeyres.herokuapp.com/api/asupan/bocil?apikey=Alphabot`)
break
...
costum(buffer, sticker, Verived, `©Created : Resta Botz`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `©Created : Resta Botz`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
for (let _ of anu) {
buttonss = [{buttonId: `${prefix}allmenu`, buttonText: {displayText: '☰ MENU'}, type: 1},{buttonId: `${prefix}sewabot`, buttonText: {displayText: '☰ SEWA BOT'}, type: 1}]
const btnbc = {
contentText: `${q}`,
